...
- (void)processResult
{
//    NSDictionary *resultDic = (self.handleredResult)[@"status"];
//    NSLog(@"%@",resultDic);
    _result = [[ITTRequestResult alloc] initWithCode:(self.handleredResult)[@"status"] withMessage:(self.handleredResult)[@"messg"]];
...
- (void)notifyDelegateRequestDidErrorWithError:(NSError*)error
{
    ...
    if (_onRequestFailedBlock) {
        ...
        [view showActivityViewInView:[UIApplication sharedApplication].keyWindow withMsg:@"联网失败,请检查网络" inSeconds:2];
