...
//                                           [conVc.navigationController pushViewController:orderListVc animated:NO];
                                           ...
                                           //app 1.4 update
                                           _orderDetail.orderId = [NSString stringWithFormat:@"%d",oWaitPost];
                                           CreateScanCodeViewController * codeVc = [[CreateScanCodeViewController alloc] init];
                                           codeVc.orderDetail = _orderDetail;
                                           [conVc.navigationController pushViewController:codeVc animated:NO];
                                       }
...
//        }];
        ...
        CreateScanCodeViewController * codeVc = [[CreateScanCodeViewController alloc] init];
        codeVc.orderDetail = _orderDetail;
        [self.navigationController pushViewController:codeVc animated:NO];
    }else{
        //交易失败
        NSLog(@"result =%@",result);
...
-(void)weiXinPay
{
    WXPayClient * wxPay = [WXPayClient shareInstance];
    [wxPay payProduct];
}
